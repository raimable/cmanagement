...
$custmerName = $daoFactory->getCustomerDAO()->load($_GET['id'])->names;
?>

<?php
if (!isset($_SESSION['query'])) {
    $_SESSION['query'] = serialize($daoFactory->getConsumptionDAO()->queryByCustomerId($_GET['id']));
    $_SESSION['view'] = serialize($custmerName);
}
...
//Check whether there are registered christian 
        ...
        if ($customer_query) {
                ...
                if ($i % 2 == 0) {
                    ...
                    echo '1';
                            ...
                            ?>">
                                    <td class="center">
                                        <?php echo $index; ?></td>
                                    <td class="normal"><?php echo $consumption->recordedOn; ?> </a></td>

                                    <td class="normal"><?php echo $consumption->amount; $totalAmount = $totalAmount + $consumption->amount; ?></td>
                                    <td class="normal"><?php echo $consumption->contractFrom; ?></td>
                                    <td class="normal"><?php echo $consumption->contractTo; ?></td>
                                    <td class="normal"><?php echo $daoFactory->getPartnerDAO()->load($consumption->partnerId)->partnerName; ?></td>
            ...
            <form method="POST" action="index.php">
                <table class="adminlist">
                    <tfoot>
                        <tr><td width="25%"><div class="left"><strong>Total Number Of records: <?php echo count($customer_query); ?></strong></div>
                                    </td>
                                    <td>
                                        <h2>Total Consumed Amount: <?php echo $totalAmount; ?></h2>
                                    </td>
                            <td>
