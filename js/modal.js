...
var SqueezeBox={
    ...
    build:function(){
        ...
        if(this.options.shadow)if(Browser.chrome||Browser.safari&&Browser.version>=3||Browser.opera&&Browser.version>=10.5||Browser.firefox&&Browser.version>=3.5||Browser.ie&&Browser.version>=9)this.win.addClass("shadow");
            else if(!Browser.ie6){
                var a=(new Element("div",{
                    "class":"sbox-bg-wrap"
                })).inject(this.win),b=function(a){
                    this.overlay.fireEvent("click",[a])
                }.bind(this);
                ["n","ne","e","se","s","sw","w","nw"].each(function(c){
                    (new Element("div",{
                        "class":"sbox-bg sbox-bg-"+
                        c
    ...
    open:function(a,b){
        this.initialize();
        this.element!=null&&this.trash();
        this.element=document.id(a)||!1;
        ...
        if(d)return this.setContent(d,this.parsers[d].call(this,!0));
        return this.parsers.some(function(a,b){
            var c=a.call(this);
            if(c)return this.setContent(b,c),!0;
            return!1
        },this)
    },
    ...
    assignOptions:function(){
        ...
        Browser.ie6&&this.win.addClass("sbox-window-ie6")
    },
    close:function(a){
        var b=typeOf(a)=="event";
        b&&a.stop();
    ...
    applyContent:function(a,b){
        if(this.isOpen||this.applyTimer)this.applyTimer=clearTimeout(this.applyTimer),this.hideContent(),a?(this.isLoading&&this.toggleLoading(!1),this.fireEvent("onUpdate",[this.content],20)):this.toggleLoading(!0),a&&(["string","array"].contains(typeOf(a))?this.content.set("html",a):a!==this.content&&this.content.contains(a)||this.content.adopt(a)),this.callChain(),this.isOpen?
            this.resize(b):(this.toggleListeners(!0),this.resize(b,!0),this.isOpen=!0,this.fireEvent("onOpen",[this.content]))
    },
    resize:function(a,b){
        this.showTimer=clearTimeout(this.showTimer||null);
        var c=this.doc.getSize(),d=this.doc.getScroll();
        ...
        c={
            width:this.size.x,
            height:this.size.y,
            left:(d.x+(c.x-this.size.x-this.options.marginInner.x)/2).toInt(),
            top:(d.y+(c.y-
                this.size.y-this.options.marginInner.y)/2).toInt()
        };
        
        this.hideContent();
        b?(this.win.setStyles(c).setStyle("display",""),this.showTimer=this.showContent.delay(50,this)):this.fx.win.start(c).chain(this.showContent.bind(this));
        return this.reposition()
    },
    ...
    toggleLoading:function(a){
        ...
        a&&this.fireEvent("onLoading",[this.win])
    },
    ...
    showContent:function(){
        this.content.get("opacity")&&this.fireEvent("onShow",[this.win]);
...
SqueezeBox.handlers.extend({
    image:function(a){
        ...
        c.onload=c.onabort=c.onerror=function(){
            ...
            if(c.width){
                ...
                for(var e=2;e--;)if(b.x>a.x)b.y*=a.x/b.x,b.x=a.x;
                    else if(b.y>a.y)b.x*=a.y/b.y,b.y=a.y;b.x=b.x.toInt();
                b.y=b.y.toInt();
    ...
    ajax:function(a){
        ...
        this.asset=(new Request.HTML(Object.merge({
            ...
            onSuccess:function(a){
                this.applyContent(a);
                b.evalScripts!==null&&!b.evalScripts&&Browser.exec(this.asset.response.javascript);
    ...
    iframe:function(a){
        ...
        if(this.options.iframePreload)return this.asset.addEvent("load",function(){
            this.applyContent(this.asset.setStyle("display",""))
        }.bind(this)),this.asset.setStyle("display","none").inject(this.content),
            !1;
        return this.asset
    },
    string:function(a){
